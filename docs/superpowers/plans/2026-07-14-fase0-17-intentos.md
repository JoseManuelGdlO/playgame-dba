...
- During the Plan 15/16 tutorial's guided tickets, deliberately submit a wrong query (e.g. a typo) at the Enviar beat: confirm the short status message appears, the Mentor dialogue stays visible and un-hidden, and — this is the part that's easy to silently break later (see the comment on `manejarClickDocumento` in `dialogo.js`) — confirm you can still click back into the SQL editor and correct the query while the Mentor's dialogue is still showing that same submit step, then resubmit successfully and confirm the tutorial continues normally to its next beat.
